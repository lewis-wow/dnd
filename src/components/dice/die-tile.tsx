...
export function DieTile({
  ...
  return (
    <button
      ...
      onClick={onAdd}
      className={cn(
        "die-tile group relative flex w-full flex-col items-center gap-2.5 rounded-2xl border border-edge bg-linear-to-br from-face-lit to-face px-2.5 pt-4.5 pb-3.5 text-center shadow-[0_6px_16px_var(--shadow),inset_0_1px_0_rgba(255,255,255,0.05)] transition-[transform,box-shadow,border-color] active:translate-y-[-1px] active:scale-[0.98] min-[900px]:gap-1 min-[900px]:px-1 min-[900px]:pt-2 min-[900px]:pb-1.5",
        qty > 0 && "border-gold",
        active &&
    ...
    >
      ...
      <div
        className={cn(
          "relative flex size-[74px] items-center justify-center max-[900px]:size-12 max-[640px]:size-[clamp(38px,12vw,50px)] min-[900px]:size-12",
          spin && "animate-spin-roll"
        )}
      >
        <DieShape die={die} crit={active ? crit : ""} />
        <span className="pointer-events-none absolute inset-0 flex items-center justify-center text-[1.4rem] font-extrabold text-foreground [text-shadow:0_1px_2px_rgba(0,0,0,0.7)] max-[900px]:text-[1.05rem] min-[900px]:text-[1.05rem]">
          {displayValue}
        </span>
      </div>
      ...
      >
        <span
          ...
          className="flex size-[18px] cursor-pointer items-center justify-center rounded-full bg-face text-[0.8rem] leading-none text-gold transition-colors hover:bg-face-lit active:scale-[0.88]"
